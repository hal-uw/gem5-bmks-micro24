...
float *alloc_1d_dbl(int n)
{
  ...
  newArr = (float *) malloc ((unsigned) (n * sizeof (float)));
  if (newArr == NULL) {
...
float **alloc_2d_dbl(int m, int n)
{
  ...
  newArr = (float **) malloc ((unsigned) (m * sizeof (float *)));
  if (newArr == NULL) {
...
BPNN *bpnn_read(char *filename)
{
  ...
  readOut = read(fd, (char *) &n1, sizeof(int));
  readOut = read(fd, (char *) &n2, sizeof(int));
  readOut = read(fd, (char *) &n3, sizeof(int));
  newArr = bpnn_internal_create(n1, n2, n3);
  ...
  memcnt = 0;
  mem = (char *) malloc ((unsigned) ((n1+1) * (n2+1) * sizeof(float)));
  readOut = read(fd, mem, (n1+1) * (n2+1) * sizeof(float));
  for (i = 0; i <= n1; i++) {
    for (j = 0; j <= n2; j++) {
      fastcopy(&(newArr->input_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
  ...
  memcnt = 0;
  mem = (char *) malloc ((unsigned) ((n2+1) * (n3+1) * sizeof(float)));
  readOut = read(fd, mem, (n2+1) * (n3+1) * sizeof(float));
  for (i = 0; i <= n2; i++) {
    for (j = 0; j <= n3; j++) {
      fastcopy(&(newArr->hidden_weights[i][j]), &mem[memcnt], sizeof(float));
      memcnt += sizeof(float);
    }
  }
  free(mem);
  ...
  bpnn_zero_weights(newArr->input_prev_weights, n1, n2);
  bpnn_zero_weights(newArr->hidden_prev_weights, n2, n3);
