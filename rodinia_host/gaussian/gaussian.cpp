...
void ForwardSub()
{
  ...
  float *m_cuda = (float *) malloc (Size * Size * sizeof(float));

  float *a_cuda = (float *) malloc (Size * Size * sizeof(float));

  float *b_cuda = (float *) malloc (Size * sizeof(float));
  ...
  gettimeofday(&time_start, NULL);
  for (t=0; t<(Size-1); t++) {
    m5_getKernelArg(reinterpret_cast<uintptr_t>(m_cuda), reinterpret_cast<uintptr_t>(a_cuda), 0, 3, 2, 1);
    hipLaunchKernelGGL(Fan1, dim3(dimGrid), dim3(dimBlock), 0, 0, m_cuda,a_cuda,Size,t);
    hipDeviceSynchronize();
     m5_getKernelArg(reinterpret_cast<uintptr_t>(m_cuda), reinterpret_cast<uintptr_t>(a_cuda), reinterpret_cast<uintptr_t>(b_cuda), 60, 3, 2);
