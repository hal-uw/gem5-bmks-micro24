...
void BFSGraph( int argc, char** argv) 
{
    ...
    //bool *h_over = (bool*) malloc (sizeof(bool));
    ...
    do
    {
      ...
      m5_getKernelArg(reinterpret_cast<uintptr_t>(h_graph_nodes), reinterpret_cast<uintptr_t>(h_graph_edges), reinterpret_cast<uintptr_t>(h_graph_mask), 48 , 3, 1);
      m5_getKernelArg(reinterpret_cast<uintptr_t>(h_updating_graph_mask), reinterpret_cast<uintptr_t>(h_graph_visited), reinterpret_cast<uintptr_t>(h_cost), 51, 3, 1);
      hipLaunchKernelGGL(Kernel, dim3(grid), dim3(threads), 0 , 0,  h_graph_nodes, h_graph_edges, h_graph_mask, h_updating_graph_mask, h_graph_visited, h_cost, no_of_nodes);
      // check if kernel execution generated and error
                
      m5_getKernelArg(reinterpret_cast<uintptr_t>(h_graph_mask), reinterpret_cast<uintptr_t>(h_updating_graph_mask), reinterpret_cast<uintptr_t>(h_graph_visited), 63 , 3, 2);
      m5_getKernelArg(reinterpret_cast<uintptr_t>(&h_over), 0, 0, 3 , 1, 2);
      hipLaunchKernelGGL(Kernel2, dim3(grid), dim3(threads), 0 , 0,  h_graph_mask, h_updating_graph_mask, h_graph_visited, &h_over, no_of_nodes);
      // check if kernel execution generated and error
