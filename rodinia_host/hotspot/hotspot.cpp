                      ...
                      int blockRows, int borderCols, int borderRows) 
{
  ...
  for (t = 0; t < total_iterations; t+=num_iterations) {
    ...
    src = dst;
    dst = temp;
    m5_getKernelArg(reinterpret_cast<uintptr_t>(MatrixPower), reinterpret_cast<uintptr_t>(MatrixTemp[src]), reinterpret_cast<uintptr_t>(MatrixTemp[dst]), 48, 3, 1);
...
void run(int argc, char** argv)
{
    ...
    float *MatrixTemp[2];
    MatrixTemp[0] = (float *) malloc (sizeof(float)*size);
    MatrixTemp[1] = (float *) malloc (sizeof(float)*size);
    memcpy(MatrixTemp[0], FilesavingTemp, sizeof(float)*size);

    float *MatrixPower = (float *) malloc (sizeof(float)*size);
