...
{
  T* t;
  //checkHIPError(hipHostMalloc((void**)&t, sizeof(T)*N));
  //hipHostMalloc((void**)&t, sizeof(T)*N);
...
{
  //checkHIPError(memcpy((void*)dst, (void*)src, N*sizeof(T)));
...
__global__ void hip_initialize_variables(int nelr, float* variables)
{
...
void initialize_variables(int nelr, float* variables)
{
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(variables), 0, 0, 3, 1, 1);
  hipLaunchKernelGGL(hip_initialize_variables, dim3(Dg), dim3(Db), 0, 0, nelr, variables);
  //    getLastHipError("initialize_variables failed");
}
...
__global__ void hip_compute_step_factor(int nelr, float* variables, float* areas, float* step_factors)
{
  const int i = (blockDim.x*blockIdx.x + threadIdx.x);
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors)
{
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(variables), reinterpret_cast<uintptr_t>(step_factors), reinterpret_cast<uintptr_t>(areas), 15, 3, 2);
  hipLaunchKernelGGL(hip_compute_step_factor, dim3(Dg), dim3(Db), 0, 0, nelr, variables, areas, step_factors);         
  //getLastHipError("compute_step_factor failed");
}
 ...
 *
*/
__global__ void hip_compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes)
{
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes)
{
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(elements_surrounding_elements), reinterpret_cast<uintptr_t>(normals), reinterpret_cast<uintptr_t>(variables), 0, 3, 3);
  m5_getKernelArg(reinterpret_cast<uintptr_t>(fluxes), 0, 0, 3, 1, 3);
  hipLaunchKernelGGL(hip_compute_flux, dim3(Dg), dim3(Db), 0, 0, nelr, elements_surrounding_elements, normals, variables, fluxes);
  //getLastHipError("compute_flux failed");
}

__global__ void hip_time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes)
{
  const int i = (blockDim.x*blockIdx.x + threadIdx.x);
...
void time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes)
{
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(variables), reinterpret_cast<uintptr_t>(old_variables), reinterpret_cast<uintptr_t>(step_factors), 3, 1, 4);
  m5_getKernelArg(reinterpret_cast<uintptr_t>(step_factors), 0, 0, 0, 1, 4);
  hipLaunchKernelGGL(hip_time_step, dim3(Dg), dim3(Db), 0, 0, j, nelr, old_variables, variables, step_factors, fluxes);
  //getLastHipError("update failed");
}
...
int main(int argc, char** argv)
{
  ...
  hipSetDevice(0);
  //checkHIPError(hipGetDevice(&dev));
  hipGetDevice(&dev);
  //checkHIPError(hipGetDeviceProperties(&prop, dev));
  ...
  // set far field conditions and load them into constant memory on the gpu
  {
    ...
    //checkHIPError( memcpyToSymbol(HIP_SYMBOL(ff_variable),          h_ff_variable,          NVAR*sizeof(float)) );
    memcpyToSymbol(HIP_SYMBOL(ff_variable),          h_ff_variable,          NVAR*sizeof(float));
    //checkHIPError( memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_x), &h_ff_flux_contribution_momentum_x, sizeof(float3)) );
    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_x), &h_ff_flux_contribution_momentum_x, sizeof(float3));
    //checkHIPError( memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_y), &h_ff_flux_contribution_momentum_y, sizeof(float3)) );
    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_y), &h_ff_flux_contribution_momentum_y, sizeof(float3));
    //checkHIPError( memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_z), &h_ff_flux_contribution_momentum_z, sizeof(float3)) );
    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_z), &h_ff_flux_contribution_momentum_z, sizeof(float3));
                
    //checkHIPError( memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_density_energy), &h_ff_flux_contribution_density_energy, sizeof(float3)) );            
    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_density_energy), &h_ff_flux_contribution_density_energy, sizeof(float3));         
  }
  int nel;
  int nelr;
  ...
  // make sure HIP isn't still doing something before we start timing
  ...
  for(int i = 0; i < iterations; i++)
  {
    ...
    for(int j = 0; j < RK; j++)
    {
      ...
      //getLastHipError("time_step failed");                   
    }
  }
  ...
  std::cout  << milliseconds / iterations << " milliseconds per iteration" << std::endl;
