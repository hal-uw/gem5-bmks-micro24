...
int main(int argc, char** argv)
{
  ...
  // set far field conditions and load them into constant memory on the gpu
  {
    ...
    memcpyToSymbol(HIP_SYMBOL(ff_variable),          h_ff_variable,          NVAR*sizeof(double));
    checkHIPError("memcpyToSymbol (ff_variable)");
    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_x), &h_ff_flux_contribution_momentum_x, sizeof(double3));
    checkHIPError("memcpyToSymbol (ff_flux_contribution_momentum_x)");
    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_y), &h_ff_flux_contribution_momentum_y, sizeof(double3));
    checkHIPError("memcpyToSymbol (ff_flux_contribution_momentum_y)");
    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_z), &h_ff_flux_contribution_momentum_z, sizeof(double3));
    checkHIPError("memcpyToSymbol (ff_flux_contribution_momentum_z)");

    memcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_density_energy), &h_ff_flux_contribution_density_energy, sizeof(double3));
    checkHIPError("memcpyToSymbol (ff_flux_contribution_density_energy)");
  }
  int nel;
  int nelr;
