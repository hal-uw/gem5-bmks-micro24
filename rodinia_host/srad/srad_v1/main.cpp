...
int main(int argc, char *argv[]) {
    ...
    jE[Nc - 1] =
        Nc - 1;  // changes IMAGE rightmost column index from Nc to Nc-1
    ...
    d_I = (fp *) malloc (mem_size);

    // allocate memory for coordinates on DEVICE
    //hipMalloc((void **)&d_iN, mem_size_i);  //
    d_iN = (int *) malloc(mem_size_i);
    memcpy(d_iN, iN, mem_size_i);           //
    d_iS = (int *) malloc(mem_size_i);
    memcpy(d_iS, iS, mem_size_i);           //
    d_jE = (int *) malloc(mem_size_i);
    memcpy(d_jE, jE, mem_size_j);           //
    d_jW = (int *) malloc(mem_size_i);
    memcpy(d_jW, jW, mem_size_j);           //

    // allocate memory for partial sums on DEVICE
    d_sums = (fp *) malloc (mem_size);
    d_sums2 = (fp *) malloc (mem_size);

    // allocate memory for derivatives
    d_dN = (fp *) malloc (mem_size);
    d_dS = (fp *) malloc (mem_size);
    d_dW = (fp *) malloc (mem_size);
    d_dE = (fp *) malloc (mem_size);

    // allocate memory for coefficient on DEVICE
    d_c = (fp *) malloc (mem_size);
    ...
    //================================================================================80
    m5_getKernelArg(reinterpret_cast<uintptr_t>(d_I), 0, 0, 3, 1, 1);
    hipLaunchKernelGGL(extract, dim3(blocks), dim3(threads), 0, 0, Ne, d_I);
    ...
    for (iter = 0; iter < niter;
        ...
        m5_getKernelArg(reinterpret_cast<uintptr_t>(d_I), reinterpret_cast<uintptr_t>(d_sums), reinterpret_cast<uintptr_t>(d_sums2), 60, 3, 2);
        hipLaunchKernelGGL(prepare, dim3(blocks), dim3(threads), 0, 0, Ne, d_I,
                           d_sums, d_sums2);
        ...
        while (blocks2.x != 0) {
            ...
            // run kernel
            m5_getKernelArg(reinterpret_cast<uintptr_t>(d_sums), reinterpret_cast<uintptr_t>(d_sums2), 0, 15, 2, 3);
            hipLaunchKernelGGL(reduce, dim3(blocks2), dim3(threads), 0, 0, Ne,
                               no, mul, d_sums, d_sums2);
        ...
        memcpy(&total, d_sums, mem_size_single);
        memcpy(&total2, d_sums2, mem_size_single);
       ...
       m5_getKernelArg(reinterpret_cast<uintptr_t>(d_iN), reinterpret_cast<uintptr_t>(d_iS), reinterpret_cast<uintptr_t>(d_jE), 0, 3, 4);
       m5_getKernelArg(reinterpret_cast<uintptr_t>(d_jW), reinterpret_cast<uintptr_t>(d_dN), reinterpret_cast<uintptr_t>(d_dS), 60, 3, 4);
       m5_getKernelArg(reinterpret_cast<uintptr_t>(d_dW), reinterpret_cast<uintptr_t>(d_dE), reinterpret_cast<uintptr_t>(d_c), 63, 3, 4);
       m5_getKernelArg(reinterpret_cast<uintptr_t>(d_I), 0, 0, 0, 1, 4);
        // execute srad kernel
        hipLaunchKernelGGL(srad, dim3(blocks), dim3(threads), 0, 0,
       ...
       m5_getKernelArg(reinterpret_cast<uintptr_t>(d_iN), reinterpret_cast<uintptr_t>(d_iS), reinterpret_cast<uintptr_t>(d_jE), 0, 3, 5);
        m5_getKernelArg(reinterpret_cast<uintptr_t>(d_jW), reinterpret_cast<uintptr_t>(d_dN), reinterpret_cast<uintptr_t>(d_dS), 60, 3, 5);
       m5_getKernelArg(reinterpret_cast<uintptr_t>(d_dW), reinterpret_cast<uintptr_t>(d_dE), reinterpret_cast<uintptr_t>(d_c), 63, 3, 5);
       m5_getKernelArg(reinterpret_cast<uintptr_t>(d_I), 0, 0, 0, 1, 5);
        // execute srad2 kernel
        hipLaunchKernelGGL(srad2, dim3(blocks), dim3(threads), 0, 0,
                           lambda,  // SRAD coefficient
                           Nr,      // # of rows in input image
                           Nc,      // # of columns in input image
                           Ne,      // # of elements in input image
    ...
    //================================================================================80
    m5_getKernelArg(reinterpret_cast<uintptr_t>(d_I), 0, 0, 3, 1, 6);
    hipLaunchKernelGGL(compress, dim3(blocks), dim3(threads), 0, 0, Ne, d_I);
    ...
    printf(
        "%15.12f s, %15.12f percent : GPU DRIVER INIT, CPU/GPU SETUP, MEMORY "
        "ALLOCATION\n",
