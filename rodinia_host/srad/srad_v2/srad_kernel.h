#ifndef SRAD_KERNEL_H_
#define SRAD_KERNEL_H_
           ...
           ) 
{
  ...
  north[ty][tx] = J_hip[index_n]; 
  south[ty][tx] = J_hip[index_s];
  if ( by == 0 ){
    north[ty][tx] = J_hip[BLOCK_SIZE * bx + tx]; 
  }
  else if ( by == hipGridDim_y - 1 ){
    south[ty][tx] = J_hip[cols * BLOCK_SIZE * (hipGridDim_y - 1) + BLOCK_SIZE * bx + cols * ( BLOCK_SIZE - 1 ) + tx];
  }
  __syncthreads();
 
  west[ty][tx] = J_hip[index_w];
  east[ty][tx] = J_hip[index_e];

  if ( bx == 0 ){
    west[ty][tx] = J_hip[cols * BLOCK_SIZE * by + cols * ty]; 
  }
  else if ( bx == hipGridDim_x - 1 ){
    east[ty][tx] = J_hip[cols * BLOCK_SIZE * by + BLOCK_SIZE * ( hipGridDim_x - 1) + cols * ty + BLOCK_SIZE-1];
  }
  __syncthreads();

  temp[ty][tx]      = J_hip[index];
            ...
            ) 
{
  ...
  __shared__ float c_hip_temp[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ float c_hip_result[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ float temp[BLOCK_SIZE][BLOCK_SIZE];
  ...
  south_c[ty][tx] = C_hip[index_s];

  if ( by == hipGridDim_y - 1 ){
    south_c[ty][tx] = C_hip[cols * BLOCK_SIZE * (hipGridDim_y - 1) + BLOCK_SIZE * bx + cols * ( BLOCK_SIZE - 1 ) + tx];
  }
  __syncthreads();

  east_c[ty][tx] = C_hip[index_e];

  if ( bx == hipGridDim_x - 1 ){
    east_c[ty][tx] = C_hip[cols * BLOCK_SIZE * by + BLOCK_SIZE * ( hipGridDim_x - 1) + cols * ty + BLOCK_SIZE-1];
  }
  __syncthreads();
  
  c_hip_temp[ty][tx]      = C_hip[index];
  __syncthreads();
  ...
  // image update (equ 61)
  c_hip_result[ty][tx] = temp[ty][tx] + 0.25 * lambda * d_sum;
  __syncthreads();
              
  J_hip[index] = c_hip_result[ty][tx];
}
