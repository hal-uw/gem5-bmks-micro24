...
int main(int argc, char *argv []){
  ...
  // HIP kernel execution parameters
  dim3 threads;
  dim3 blocks;
  ...
  // pointers
  common_change.d_frame = (float *) malloc (sizeof(common.frame_mem));
  ...
  common.d_endoRow = (int *) malloc (sizeof(common.endo_mem));
  ...
  common.d_endoCol = (int *) malloc (sizeof(common.endo_mem));
  memcpy(common.d_endoCol, common.endoCol, common.endo_mem);

  common.tEndoRowLoc = (int *)malloc(common.endo_mem * common.no_frames);
  common.d_tEndoRowLoc = (int *) malloc (sizeof( common.endo_mem * common.no_frames));

  common.tEndoColLoc = (int *)malloc(common.endo_mem * common.no_frames);
  common.d_tEndoColLoc = (int *) malloc (sizeof( common.endo_mem * common.no_frames));
  ...
  common.d_epiRow = (int *) malloc (sizeof( common.epi_mem));
  ...
  common.d_epiCol = (int *) malloc (sizeof( common.epi_mem));
  memcpy(common.d_epiCol, common.epiCol, common.epi_mem);

  common.tEpiRowLoc = (int *)malloc(common.epi_mem * common.no_frames);
  common.d_tEpiRowLoc = (int *) malloc (sizeof( common.epi_mem * common.no_frames));

  common.tEpiColLoc = (int *)malloc(common.epi_mem * common.no_frames);
  common.d_tEpiColLoc = (int *) malloc (sizeof( common.epi_mem * common.no_frames));
  ...
  // common
  common.d_endoT = (float *) malloc (sizeof( common.in_mem * common.endoPoints));
  common.d_epiT = (float *) malloc (sizeof( common.in_mem * common.epiPoints));
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2 = (float *) malloc (sizeof( common.in2_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_conv = (float *) malloc (sizeof( common.conv_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2_pad_cumv = (float *) malloc (sizeof( common.in2_pad_cumv_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2_pad_cumv_sel = (float *) malloc (sizeof( common.in2_pad_cumv_sel_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2_sub_cumh = (float *) malloc (sizeof( common.in2_sub_cumh_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2_sub_cumh_sel = (float *) malloc (sizeof( common.in2_sub_cumh_sel_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2_sub2 = (float *) malloc (sizeof( common.in2_sub2_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2_sqr = (float *) malloc (sizeof( common.in2_sqr_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in2_sqr_sub2 = (float *) malloc (sizeof( common.in2_sqr_sub2_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_in_sqr = (float *) malloc (sizeof( common.in_sqr_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_tMask = (float *) malloc (sizeof( common.tMask_mem));
  }
  ...
  // pointers
  for(i=0; i<common.allPoints; i++){
    unique[i].d_mask_conv = (float *) malloc (sizeof( common.mask_conv_mem));
  }
  ...
  //    COPY ARGUMENTS
  //====================================================================================================
  memcpy(&d_common, &common, sizeof(params_common));
  memcpy(&d_unique, &unique, sizeof(params_unique)*ALL_POINTS);
  ...
  for(common_change.frame_no=0; common_change.frame_no<frames_processed; common_change.frame_no++){
    ...
    memcpy(&d_common_change, &common_change, sizeof(params_common_change));

    // launch GPU kernel
    hipLaunchKernelGGL(kernel1, dim3(blocks), dim3(threads), 0, 0);
  ...
  free(common.d_endoRow);
  free(common.d_endoCol);
  free(common.d_tEndoRowLoc);
  free(common.d_tEndoColLoc);
  ...
  free(common.d_epiRow);
  free(common.d_epiCol);
  free(common.d_tEpiRowLoc);
  free(common.d_tEpiColLoc);
  ...
  for(i=0; i<common.allPoints; i++){
    ...
    free(unique[i].d_conv);
    free(unique[i].d_in2_pad_cumv);
    free(unique[i].d_in2_pad_cumv_sel);
    free(unique[i].d_in2_sub_cumh);
    free(unique[i].d_in2_sub_cumh_sel);
    free(unique[i].d_in2_sub2);
    free(unique[i].d_in2_sqr);
    free(unique[i].d_in2_sqr_sub2);
    free(unique[i].d_in_sqr);

    free(unique[i].d_tMask);
    free(unique[i].d_mask_conv);
  }
}
