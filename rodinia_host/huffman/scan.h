...
static void preallocBlockSums(unsigned int maxNumElements)
{
    ...
    do {       
        unsigned int numBlocks = max(1, (int)ceil((float)numElts / (2.f * blockSize)));
        if (numBlocks > 1)
            g_scanBlockSums[level++] = (unsigned int*) malloc (numBlocks * sizeof(unsigned int));
                           ...
                           int level)
{
    ...
    if (numBlocks > 1)
    {
        m5_getKernelArg(reinterpret_cast<uintptr_t>(outArray), reinterpret_cast<uintptr_t>(inArray), 0, 3, 2, 1);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<true, false>), dim3(grid), dim3(threads), sharedMemSize , 0, outArray, 
                                                                 inArray, 
        ...
        if (np2LastBlock)
        {
            m5_getKernelArg(reinterpret_cast<uintptr_t>(outArray), reinterpret_cast<uintptr_t>(inArray), 0, 3, 2, 2);
            hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<true, true>), dim3(1), dim3(numThreadsLastBlock), sharedMemLastBlock , 0, outArray, inArray, g_scanBlockSums[level], numEltsLastBlock, 
        ...
        prescanArrayRecursive(g_scanBlockSums[level], 
                              ...
                              numBlocks, 
                              level+1);
        m5_getKernelArg(reinterpret_cast<uintptr_t>(outArray), 0, 0, 3, 1, 3);
        ...
        if (np2LastBlock)
        {
            m5_getKernelArg(reinterpret_cast<uintptr_t>(outArray), 0, 0, 3, 1, 4);
    ...
    {
        m5_getKernelArg(reinterpret_cast<uintptr_t>(outArray), reinterpret_cast<uintptr_t>(inArray), 0, 3, 2, 5);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<false, false>), dim3(grid), dim3(threads), sharedMemSize , 0, outArray, inArray,
                                                                  (unsigned int*)NULL, numThreads * 2, 0, 0);
        CUT_CHECK_ERROR("prescan");
    }
    else
    {
        m5_getKernelArg(reinterpret_cast<uintptr_t>(outArray), reinterpret_cast<uintptr_t>(inArray), 0, 3, 2, 6);
         hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<false, true>), dim3(grid), dim3(threads), sharedMemSize , 0, outArray, inArray, 
