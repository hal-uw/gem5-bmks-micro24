...
int runHisto(char* file, unsigned int* freq, unsigned int memSize, unsigned int *source) {
    ...
    */
    ...
    for(int i = 0; i < totalNum; i+=partialNum*2)
    {
        ...
        hipLaunchKernelGGL(histo_kernel, dim3(blocks*2), dim3(256), 0, stream0,  buffer+i, partSize, freq );
        hipLaunchKernelGGL(histo_kernel, dim3(blocks*2), dim3(256), 0, stream1,  buffer+i+partialNum, partSize, freq );
    }
