...
__global__ void vlc_encode_kernel_sm64huff(unsigned int* data,
                                                  unsigned int* gm_codewords, unsigned int* gm_codewordlens,
#ifdef TESTING
                                                  unsigned int* cw32, unsigned int* cw32len, unsigned int* cw32idx, 
#endif
                                                  unsigned int* out, unsigned int *outidx){

  unsigned int kn = hipBlockIdx_x*hipBlockDim_x + hipThreadIdx_x;
  unsigned int k = hipThreadIdx_x;
  ...
  HIP_DYNAMIC_SHARED( unsigned int, sm)
  ...
  for (unsigned int d = (hipBlockDim_x)>>1; d > 0; d >>= 1)  {
    __syncthreads();
    if (k < d)   {
  ...
  if (k == 0) as[hipBlockDim_x - 1] = 0;    

  // traverse down the tree building the scan in place
  for (unsigned int d = 1; d < hipBlockDim_x; d *= 2)    {
    offset >>= 1;
    __syncthreads();
    if (k < d)   {
      unsigned char ai = offset*(2*k+1)-1;
  ...
  if (k == hipBlockDim_x-1) {
    outidx[hipBlockIdx_x] = as[k] + codewordlen;
