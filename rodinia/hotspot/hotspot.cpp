                      ...
                      int blockRows, int borderCols, int borderRows, size_t numStreams, hipStream_t* hip_stream) 
{
  ...
  for (int i = 0; i < numStreams; ++i) {
    for (t = 0; t < total_iterations; t+=num_iterations) {
      ...
      hipLaunchKernelGGL(calculate_temp, dim3(dimGrid), dim3(dimBlock), 0, hip_stream[i] , MIN(num_iterations, total_iterations-t), MatrixPower[i],MatrixTemp[src][i],MatrixTemp[dst][i],\
