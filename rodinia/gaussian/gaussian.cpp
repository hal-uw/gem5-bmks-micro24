...
void ForwardSub(int argc, char** argv)
{
  ...
  for (int i = 0; i < numStreams; ++i) {
    for (t=0; t<(Size-1); t++) {
      ...
      hipLaunchKernelGGL(HIP_KERNEL_NAME(Fan1), dim3(dimGrid), dim3(dimBlock), 0, hip_stream[i], /*0,*/ &m_cuda[i][0],&a_cuda[i][0],Size,t);
  ...
  for (int i = 0; i < numStreams; ++i) {
    for (t=0; t<(Size-1); t++) {
      ...
      hipLaunchKernelGGL(HIP_KERNEL_NAME(Fan2), dim3(dimGridXY), dim3(dimBlockXY), 0, hip_stream[i],/* 0,*/ &m_cuda[i][0], &a_cuda[i][0], &b_cuda[i][0],Size,Size-t,t);
      hipHccModuleRingDoorbell(hip_stream[i]);
      hipStreamSynchronize(hip_stream[i]);
      m5_dump_reset_stats(0, 0);
    }
