...
int main(int argc, char* argv[])
{
  ...
  //LatLong **d_locations;
  vector<LatLong*> d_locations(numStreams);
  vector<float*> d_distances(numStreams);
  //float **d_distances;
  ...
  hipDeviceSynchronize();
  unsigned long usableDeviceMemory = freeDeviceMemory * 85 / 100; // 85% arbitrary throttle to compensate for known HIP bug
  ...
  for (int i = 0; i < numStreams; i++) {
    //d_locations[i] = (LatLong*)malloc(sizeof(LatLong) * numRecords);
    hipHostMalloc((void **) &d_locations[i],sizeof(LatLong) * numRecords);
  }

  //hipHostMalloc((void **) &d_locations,sizeof(LatLong*) * numStreams);
  //for (int i = 0; i < numStreams; i++) {
  //  hipHostMalloc((void **) &d_locations[i],sizeof(LatLong) * numRecords);
  //}
  //hipHostMalloc((void **) &d_distances,sizeof(float*) * numStreams);
  for (int i = 0; i < numStreams; i++) {
    //d_distances[i] = (float*)malloc(sizeof(float) * numRecords);
    hipHostMalloc((void **) &d_distances[i],sizeof(float) * numRecords);
  }
   ...
   */
  for (int i = 0; i < numStreams; i++) {
    hipMemcpy( d_locations[i], &locations[0], sizeof(LatLong) * numRecords, hipMemcpyHostToDevice);
  }
   ...
   */
   int count = 1;
  for (int i = 0; i < numStreams; i++) {
     m5_getKernelArg(reinterpret_cast<uintptr_t>(d_locations[i]), reinterpret_cast<uintptr_t>(d_distances[i]), 0, 0, 2, count++);
    hipLaunchKernelGGL(euclid, dim3(gridDim), dim3(threadsPerBlock ), 0, hip_stream[i], /*0,*/ d_locations[i],d_distances[i],numRecords,lat,lng);
  }
  for (int i = 0; i < numStreams; i++) {
  ...
  hipMemcpy( distances, d_distances[0], sizeof(float)*numRecords, hipMemcpyDeviceToHost );
...
int loadData(char *filename,std::vector<Record> &records,std::vector<LatLong> &locations){
  ...
  while(!feof(flist)) {
     ...
     */
    if(fscanf(flist, "%s\n", dbname) != 1) {
      fprintf(stderr, "error reading filelist: %s\n", dbname);
      exit(0);
    }
    fp = fopen(dbname, "r");
    if(!fp) {
