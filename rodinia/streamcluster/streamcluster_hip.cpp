...
void allocDevMem(int num, int dim)
{
  ...
  HIP_SAFE_CALL( hipHostMalloc((void**) &center_table_d,    sizeof(int*)*numStreams ));
  HIP_SAFE_CALL( hipHostMalloc((void**) &switch_membership_d, sizeof(bool*)*numStreams));
  HIP_SAFE_CALL( hipHostMalloc((void**) &p,  sizeof(Point*)*numStreams));
  HIP_SAFE_CALL( hipHostMalloc((void**) &coord_d, sizeof(float*)*numStreams));
  for (int i = 0; i < numStreams; ++i) {
    HIP_SAFE_CALL( hipHostMalloc((void**) &center_table_d[i],    num * sizeof(int))   );
    HIP_SAFE_CALL( hipHostMalloc((void**) &switch_membership_d[i], num * sizeof(bool))  );
    HIP_SAFE_CALL( hipHostMalloc((void**) &p[i], num * sizeof(Point)) );
    HIP_SAFE_CALL( hipHostMalloc((void**) &coord_d[i], num * dim * sizeof(float)) );
  }
}
...
{
  for (int i = 0; i < numStreams; ++i) {
     hipFree(center_table_d[i])        ;
     hipFree(switch_membership_d[i]) ;
     hipFree(p[i])                                     ;
     hipFree(coord_d[i])                       ;
  }
}
...
{       
  ...
  *serial_t += (double) tmp_t;
  ...
  // ALLOCATE GPU MEMORY
  //=======================================
  HIP_SAFE_CALL( hipHostMalloc((void**) &work_mem_d, sizeof(float*)*numStreams) );
  for (int i = 0; i < numStreams; ++i) { 
    HIP_SAFE_CALL( hipHostMalloc((void**) &work_mem_d[i],  stride * (nThread + 1) * sizeof(float)) );
  }
  ...
  *alloc_t += (double) tmp_t;
  ...
  for (int i = 0; i < numStreams; ++i) { 
    if(isCoordChanged || iter == 0)
    {
      HIP_SAFE_CALL( hipMemcpy(coord_d[i],  coord_h,        num * dim * sizeof(float), hipMemcpyHostToDevice) );
    }
    HIP_SAFE_CALL( hipMemcpy(center_table_d[i],  center_table,  num * sizeof(int),   hipMemcpyHostToDevice) );
    HIP_SAFE_CALL( hipMemcpy(p[i],  points->p,                                num * sizeof(Point), hipMemcpyHostToDevice) );
          
    HIP_SAFE_CALL( hipMemset((void*) switch_membership_d[i], 0,                    num * sizeof(bool))  );
    HIP_SAFE_CALL( hipMemset((void*) work_mem_d[i],                0, stride * (nThread + 1) * sizeof(float)) );
  }      
#ifdef HIPTIME
  hipEventRecord(stop,0);
  hipEventSynchronize(stop);
...
int countKernel = 1;
  for (int i = 0; i < numStreams; ++i) { 
  m5_getKernelArg(reinterpret_cast<uintptr_t>(p[i]), reinterpret_cast<uintptr_t>(coord_d[i]), reinterpret_cast<uintptr_t>(work_mem_d[i]), 48, 3, countKernel);
  m5_getKernelArg(reinterpret_cast<uintptr_t>(center_table_d[i]), reinterpret_cast<uintptr_t>(switch_membership_d[i]), 0, 12, 2, countKernel++);
  hipLaunchKernelGGL(kernel_compute_cost, dim3(grid_size), dim3(THREADS_PER_BLOCK), 0, hip_stream[i], /*0,*/         
                                                        num,                                    // in:  # of data
                                                        dim,                                    // in:  dimension of point coordinates
                                                        x,                                              // in:  point to open a center at
                                                        p[i],                                              // in:  data point array
                                                        K,                                              // in:  number of centers
                                                        stride,                                 // in:  size of each work_mem segment
                                                        coord_d[i],                                // in:  array of point coordinates
                                                        work_mem_d[i],                             // out: cost and lower field array
                                                        center_table_d[i],                 // in:  center index table
                                                        switch_membership_d[i]             // out:  changes in membership
                                                                );
  }      
  ...
  *kernel_t += (double) tmp_t;
  ...
  //=======================================
  HIP_SAFE_CALL( hipMemcpy(work_mem_h,                  work_mem_d[0],   stride * (nThread + 1) * sizeof(float), hipMemcpyDeviceToHost) );
  HIP_SAFE_CALL( hipMemcpy(switch_membership, switch_membership_d[0],     num * sizeof(bool),  hipMemcpyDeviceToHost) );
  ...
  *serial_t += (double) tmp_t;
  ...
  //=======================================
  for (int i = 0; i < numStreams; ++i) {
//HIP_SAFE_CALL( hipFree(work_mem_d[i]) );
   hipFree(work_mem_d[i]);
  }
