...
for (int i = 0; i < numStreams; i++) { 
    ...
    //Run kernels
    hipLaunchKernelGGL(srad_hip_1, dim3(dimGrid), dim3(dimBlock), 0, hip_stream[i], /*0,*/ E_C[i], W_C[i], N_C[i], S_C[i], J_hip[i], C_hip[i], cols, rows, q0sqr);
}  
for (int i = 0; i < numStreams; ++i) { 
...
for (int i = 0; i < numStreams; ++i) { 
    ...
    hipLaunchKernelGGL(srad_hip_2, dim3(dimGrid), dim3(dimBlock), 0, hip_stream[i], /* 0,*/ E_C[i], W_C[i], N_C[i], S_C[i], J_hip[i], C_hip[i], cols, rows, lambda, q0sqr); 
  }
