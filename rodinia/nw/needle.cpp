...
void runTest( int argc, char** argv) 
{
  ...
  for (int j = 0; j < numStreams; ++j) {
    for( int i = 1 ; i <= block_width ; i++){
      ...
      hipLaunchKernelGGL(needle_hip_shared_1, dim3(dimGrid), dim3(dimBlock), 0, hip_stream[j], /*0,*/ referrence_hip[j], matrix_hip[j]
  ...
  for (int j = 0; j < numStreams; ++j) {
    for( int i = block_width - 1  ; i >= 1 ; i--){
      ...
      dimGrid.y = 1;
      m5_getKernelArg(reinterpret_cast<uintptr_t>(referrence_hip[j]), reinterpret_cast<uintptr_t>(matrix_hip[j]), 0, 12, 2, count++);
      hipLaunchKernelGGL(needle_hip_shared_2, dim3(dimGrid), dim3(dimBlock), 0, hip_stream[j], /*0,*/ referrence_hip[j], matrix_hip[j]
                                                  ,max_cols, penalty, i, block_width); 
      hipHccModuleRingDoorbell(hip_stream[i]);
    hipStreamSynchronize(hip_stream[i]);
    m5_dump_reset_stats(0, 0);
