...
__global__ void hip_initialize_variables(int nelr, double* variables)
{
...
__global__ void hip_compute_step_factor(int nelr, double* variables, double* areas, double* step_factors)
{
  const int i = (blockDim.x*blockIdx.x + threadIdx.x);
 ...
 *
*/
__global__ void hip_compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fluxes)
{
...
__global__ void hip_time_step(int j, int nelr, double* old_variables, double* variables, double* step_factors, double* fluxes)
{
  const int i = (blockDim.x*blockIdx.x + threadIdx.x);
...
int main(int argc, char** argv)
{
  ...
  hipSetDevice(0);
  checkHIPError("hipSetDevice");
  hipGetDevice(&dev);
  checkHIPError("hipGetDevice(&dev)");
  hipGetDeviceProperties(&prop, dev);
  checkHIPError("hipGetDeviceProperties");
  ...
  // set far field conditions and load them into constant memory on the gpu
  {
    ...
    hipMemcpyToSymbol(HIP_SYMBOL(ff_variable),          h_ff_variable,          NVAR*sizeof(double));
    checkHIPError("hipMemcpyToSymbol (ff_variable)");
    hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_x), &h_ff_flux_contribution_momentum_x, sizeof(double3));
    checkHIPError("hipMemcpyToSymbol (ff_flux_contribution_momentum_x)");
    hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_y), &h_ff_flux_contribution_momentum_y, sizeof(double3));
    checkHIPError("hipMemcpyToSymbol (ff_flux_contribution_momentum_y)");
    hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_z), &h_ff_flux_contribution_momentum_z, sizeof(double3));
    checkHIPError("hipMemcpyToSymbol (ff_flux_contribution_momentum_z)");

    hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_density_energy), &h_ff_flux_contribution_density_energy, sizeof(double3));
    checkHIPError("hipMemcpyToSymbol (ff_flux_contribution_density_energy)");
  }
  int nel;
  int nelr;
