...
{
  //checkHIPError(hipFree((void*)array));
  hipFree((void*)array);
}
...
{
  //checkHIPError(hipMemcpy((void*)dst, (void*)src, N*sizeof(T), hipMemcpyDeviceToDevice));
...
{
  //checkHIPError(hipMemcpy((void*)dst, (void*)src, N*sizeof(T), hipMemcpyHostToDevice));
...
{
  //checkHIPError(hipMemcpy((void*)dst, (void*)src, N*sizeof(T), hipMemcpyDeviceToHost));
...
hipStreamCreateWithFlags(&hip_stream, 0x01, -1);
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(variables), 0, 0, 3, 1, count++);
  hipLaunchKernelGGL_lk(hip_initialize_variables, dim3(Dg), dim3(Db), 0, hip_stream, nelr, variables);
...
hipStreamCreateWithFlags(&hip_stream, 0x01, -1);
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(variables), reinterpret_cast<uintptr_t>(step_factors), reinterpret_cast<uintptr_t>(areas), 15, 3, count++);
  hipLaunchKernelGGL_lk(hip_compute_step_factor, dim3(Dg), dim3(Db), 0, hip_stream, nelr, variables, areas, step_factors);         
...
hipStreamCreateWithFlags(&hip_stream, 0x01, -1);
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(elements_surrounding_elements), reinterpret_cast<uintptr_t>(normals), reinterpret_cast<uintptr_t>(variables), 0, 3, count);
  m5_getKernelArg(reinterpret_cast<uintptr_t>(fluxes), 0, 0, 3, 1, count++);
  hipLaunchKernelGGL_lk(hip_compute_flux, dim3(Dg), dim3(Db), 0, hip_stream, nelr, elements_surrounding_elements, normals, variables, fluxes);
...
hipStreamCreateWithFlags(&hip_stream, 0x01, -1);
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(variables), reinterpret_cast<uintptr_t>(old_variables), reinterpret_cast<uintptr_t>(step_factors), 3, 1, count);
  m5_getKernelArg(reinterpret_cast<uintptr_t>(step_factors), 0, 0, 0, 1, count++);
  hipLaunchKernelGGL_lk(hip_time_step, dim3(Dg), dim3(Db), 0, hip_stream, j, nelr, old_variables, variables, step_factors, fluxes);
...
int main(int argc, char** argv)
{
  ...
  // set far field conditions and load them into constant memory on the gpu
  {
    ...
    //checkHIPError( hipMemcpyToSymbol(HIP_SYMBOL(ff_variable),          h_ff_variable,          NVAR*sizeof(float)) );
    hipMemcpyToSymbol(HIP_SYMBOL(ff_variable),          h_ff_variable,          NVAR*sizeof(float));
    //checkHIPError( hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_x), &h_ff_flux_contribution_momentum_x, sizeof(float3)) );
    hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_x), &h_ff_flux_contribution_momentum_x, sizeof(float3));
    //checkHIPError( hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_y), &h_ff_flux_contribution_momentum_y, sizeof(float3)) );
    hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_y), &h_ff_flux_contribution_momentum_y, sizeof(float3));
    //checkHIPError( hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_z), &h_ff_flux_contribution_momentum_z, sizeof(float3)) );
    hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_z), &h_ff_flux_contribution_momentum_z, sizeof(float3));
                
    //checkHIPError( hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_density_energy), &h_ff_flux_contribution_density_energy, sizeof(float3)) );            
