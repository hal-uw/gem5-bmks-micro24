...
void InitPerRun();
void ForwardSub(int argc, char** argv);
void BackSub();
...
int main(int argc, char *argv[])
{
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
    ...
    if (argc < 2) {
  	...
  	printf("Usage: gaussian -f filename / -s size [-q] <numStream> <IndividualGPUs>\n\n");
...
void PrintDeviceProperties(){
  ...
  {  
    ...
    if( hipSuccess == hipGetDeviceProperties(&deviceProp, nDeviceIdx))  
    {
      ...
      // As of ROCm 4.0.1, not supported in hipGetDeviceProperties
...
void InitProblemOnce(char *filename)
{
  ...
  fp = fopen(filename, "r");
  printf("Rajesh:File open done\n");

  fscanf(fp, "%d", &Size);

  a = (float *) malloc(Size * Size * sizeof(float));
  printf("Rajesh:Malloc File done\n");
...
{   
  ...
  //printf("hipBlockIdx_x:%d,hipThreadIdx_x:%d,Size:%d,t:%d,Size-1-t:%d\n",hipBlockIdx_x,hipThreadIdx_x,Size,t,Size-1-t);
...
void ForwardSub(int argc, char** argv)
{
  ...
  // copy memory to GPU
  hipHostMalloc((void **) &m_cuda, sizeof(float*)*numStreams);
  for (int i = 0; i < numStreams; ++i) {
  	hipHostMalloc((void **) &m_cuda[i], Size * Size * sizeof(float));
	hipMemcpy( &m_cuda[i][0], m, Size * Size * sizeof(float), hipMemcpyHostToDevice); 
  }
  
  hipHostMalloc((void **) &a_cuda, sizeof(float*)*numStreams);
  for (int i = 0; i < numStreams; ++i) {
  	hipHostMalloc((void **) &a_cuda[i], Size * Size * sizeof(float));
	hipMemcpy( &a_cuda[i][0], a, Size * Size * sizeof(float), hipMemcpyHostToDevice); 
  }
  
  hipHostMalloc((void **) &b_cuda, sizeof(float*)*numStreams);
  for (int i = 0; i < numStreams; ++i) {
  	hipHostMalloc((void **) &b_cuda[i], Size * sizeof(float));
	hipMemcpy( &b_cuda[i][0], b, Size * sizeof(float), hipMemcpyHostToDevice); 
  }
  ...
  for (int i = 0; i < numStreams; ++i) {
    for (t=0; t<(Size-1); t++) {
      m5_getKernelArg(reinterpret_cast<uintptr_t>(&m_cuda[i][0]), reinterpret_cast<uintptr_t>(&a_cuda[i][0]), 0, 3, 2, count++);
      hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(Fan1), dim3(dimGrid), dim3(dimBlock), 0, hip_stream[i], 0, &m_cuda[i][0],&a_cuda[i][0],Size,t);
    }
  }
  ...
  for (int i = 0; i < numStreams; ++i) {
    for (t=0; t<(Size-1); t++) {
      m5_getKernelArg(reinterpret_cast<uintptr_t>(&m_cuda[i][0]), reinterpret_cast<uintptr_t>(&a_cuda[i][0]), reinterpret_cast<uintptr_t>(&b_cuda[i][0]), 60, 3, count++);
      hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(Fan2), dim3(dimGridXY), dim3(dimBlockXY), 0, hip_stream[i],0, &m_cuda[i][0], &a_cuda[i][0], &b_cuda[i][0],Size,Size-t,t);
    }
  }
  ...
  hipMemcpy(m, &m_cuda[0][0], Size * Size * sizeof(float),hipMemcpyDeviceToHost );
  hipMemcpy(a, &a_cuda[0][0], Size * Size * sizeof(float),hipMemcpyDeviceToHost );
  hipMemcpy(b, &b_cuda[0][0], Size * sizeof(float),hipMemcpyDeviceToHost );
  hipFree(m_cuda);
  hipFree(a_cuda);
  hipFree(b_cuda);
}
