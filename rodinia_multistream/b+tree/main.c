...
#include "util/hip/hip.h"                              // (in directory provided here)

//======================================================================================================================================================150
//      KERNEL HEADERS
//======================================================================================================================================================150
#include "kernel/kernel_gpu_hip_wrapper.h"             // (in directory provided here)
#include "kernel/kernel_gpu_hip_wrapper.hip.h"         // (in directory provided here)
#include "./kernel/kernel_gpu_hip_wrapper_2.h"         // (in directory provided here)
#include "./kernel/kernel_gpu_hip_wrapper_2.hip.h"     // (in directory provided here)
        ...
        char** argv ) 
{
  ...
  for(cur_arg=1; cur_arg<argc; cur_arg++){
    ...
    if(strcmp(argv[cur_arg], "file")==0){
      ...
      // value not provided
        ...
        numStreams = atoi(argv[cur_arg]);
        cur_arg = cur_arg+1;
        individualGpus = atoi(argv[cur_arg]);
    }
  ...
  long mem_used = transform_to_hip(root,0);
  ...
  while (sscanf(commandPointer, "%c", &instruction) != EOF) {
    ...
    switch (instruction) {
      ...
      case 'k':
      {
        ...
        printf("\n ******command: k count=%d \n",count);
        if(count > 65535){
          printf("ERROR: Number of requested querries should be 65,535 at most. (limited by # of HIP blocks)\n");
          exit(0);
        }
        ...
        // HIP kernel
        kernel_gpu_hip_wrapper(records,
                               records_mem,
                               knodes,
                               knodes_elem,
                               knodes_mem,
      ...
      case 'j':
      {
        ...
        // HIP kernel
        kernel_gpu_hip_wrapper_2(      knodes,
                                       knodes_elem,
                                       knodes_mem,
