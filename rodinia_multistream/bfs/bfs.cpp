...
void BFSGraph( int argc, char** argv) 
{
	...
	stop_perStream[i] = false;
        ...
        hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(Kernel), dim3(grid), dim3(threads), 0 , hip_stream[i], 0,
  ...
  m5_getKernelArg(reinterpret_cast<uintptr_t>(&d_over[i]), 0, 0, 3 , 1, count++);       
	hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(Kernel2), dim3(grid), dim3(threads), 0 , hip_stream[i],0,
