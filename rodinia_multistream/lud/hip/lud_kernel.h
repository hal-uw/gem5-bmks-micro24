...
for (int j = 0; j < numStreams; ++j) {
      m5_getKernelArg(reinterpret_cast<uintptr_t>(m[j]), 0, 0, 3, 1, count++);
      hipLaunchKernelGGL_lk(lud_diagonal, dim3(1), dim3(BLOCK_SIZE), 0, hip_stream[j], 0, m[j], matrix_dim, i);
}
...
for (int j = 0; j < numStreams; ++j) {
    m5_getKernelArg(reinterpret_cast<uintptr_t>(m[j]), 0, 0, 3, 1, count++); 
      hipLaunchKernelGGL_lk(lud_perimeter, dim3((matrix_dim-i)/BLOCK_SIZE-1), dim3(BLOCK_SIZE*2), 0, hip_stream[j], 0, m[j], matrix_dim, i);
}
...
for (int j = 0; j < numStreams; ++j) { 
      ...
      m5_getKernelArg(reinterpret_cast<uintptr_t>(m[j]), 0, 0, 3, 1, count++);
      hipLaunchKernelGGL_lk(lud_internal, dim3(dimGrid), dim3(dimBlock), 0, hip_stream[j], 0, m[j], matrix_dim, i); 
}
