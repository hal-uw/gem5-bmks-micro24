#ifndef NEEDLE_KERNEL_H_
#define NEEDLE_KERNEL_H_
...
__global__ void
needle_hip_shared_1(  int* referrence,
                      int* matrix_hip, 
                      int cols,
                      int penalty,
                      int i,
                      int block_width) 
{
  ...
  temp[tx + 1][0] = matrix_hip[index_w + cols * tx];
  __syncthreads();

  temp[0][tx + 1] = matrix_hip[index_n];
  __syncthreads();
  ...
  for ( int ty = 0 ; ty < BLOCK_SIZE ; ty++) {
    matrix_hip[index + ty * cols] = temp[ty+1][tx+1];
  }
}

__global__ void
needle_hip_shared_2(  int* referrence,
                      int* matrix_hip, 
