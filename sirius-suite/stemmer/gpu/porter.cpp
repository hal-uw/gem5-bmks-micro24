...
int main(int argc, char *argv[]) {
  ...
  hipEventRecord(eStart, 0);
  m5_getKernelArg(reinterpret_cast<uintptr_t>(gpu_stem_list), reinterpret_cast<uintptr_t>(words), 0, 3, 2, 1);
  hipLaunchKernelGGL(HIP_KERNEL_NAME(stem_gpu), dim3(grid), dim3(block), 0, 0, gpu_stem_list, words);
  hipEventRecord(eStop, 0);
  hipEventSynchronize(eStop);
  hipEventElapsedTime(&cuda_elapsedTime, eStart, eStop);
