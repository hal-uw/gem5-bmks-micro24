...
int main(int argc, char *argv[]) {
  ...
  //hipHostMalloc((void **)&stem_list, ARRAYSIZE* sizeof(struct stemmer));
  ...
  for (i = 0; i < GPU_N; i++)
  {
      ...
      //plan[i].h_Data = (struct stemmer*) malloc(plan[i].dataN * sizeof(struct stemmer));
      //for (int j = 0; j < plan[i].dataN; j++)
      //{
      //    plan[i].h_Data[j] = stem_list[i*plan[i].dataN+j];
      //}
      hipMalloc((void **)&plan[i].h_Data, plan[i].dataN * sizeof(struct stemmer));
      hipMemcpyAsync(plan[i].h_Data, &stem_list[i*plan[i].dataN], plan[i].dataN * sizeof(struct stemmer), hipMemcpyHostToDevice, plan[i].stream);
  }
  ...
  for (i=0; i < GPU_N; i++)
  {
      ...
      dim3 block(256);
      dim3 grid;
      grid.x = (plan[i].dataN + block.x - 1) / block.x;
      //grid.x = ceil(plan[i].dataN * 1.0 / block.x);
      m5_getKernelArg(reinterpret_cast<uintptr_t>(plan[i].h_Data), reinterpret_cast<uintptr_t>(plan[i].dataN), 0, 3, 2, 1);
      hipLaunchKernelGGL(HIP_KERNEL_NAME(stem_gpu), dim3(grid), dim3(block), 0, plan[i].stream, plan[i].h_Data, plan[i].dataN);
  }
  ...
  {
      hipSetDevice(i);
      hipMemcpyAsync(&stem_list[i*plan[i].dataN], plan[i].h_Data, plan[i].dataN * sizeof(struct stemmer), hipMemcpyDeviceToHost, plan[i].stream);
      hipStreamSynchronize(plan[i].stream);
  ...
  //STATS_END();
#ifdef TESTING
  f = fopen(("./stem_porter_" + std::to_string(GPU_N) + ".gpu").c_str(), "w");
