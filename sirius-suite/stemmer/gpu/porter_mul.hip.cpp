...
__global__ void stem_gpu(struct stemmer *stem_list, int words, int start) {
  int tid = hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x + start;
  if (tid < words) {
...
int main(int argc, char *argv[]) {
  ...
  for (i=0; i < GPU_N; i++) {
    ...
    grid.x = ceil(dataN * 1.0 / block.x);
    int start = i*dataN;
     m5_getKernelArg(reinterpret_cast<uintptr_t>(stem_list), reinterpret_cast<uintptr_t>(words), 0, 3, 2, 1);
    hipLaunchKernelGGL(HIP_KERNEL_NAME(stem_gpu), dim3(grid), dim3(block), 0, 0, stem_list, words, start);
  }
