...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < numStreams; i++) {
    ...
    hipStreamCreateWithFlags(&hip_stream[i]); // , 0x01, 1049120998);
  }
  ...
  for (int i = 0; i < numStreams; i++) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(stem_gpu), dim3(grid), dim3(block), 0,
                       hip_stream[i], 0, stem_list, words);
  }
