...
int main(int argc, char **argv) {
    ...
    // Support for user-fed hyperparameters
    if(argc < 5){
        std::cout << "Usage: " << argv[0] << " type (vanilla, lstm, gru) hidden_state batch_size time_steps [training/inference] [precision]" << std::endl;
        exit(1);
    }
