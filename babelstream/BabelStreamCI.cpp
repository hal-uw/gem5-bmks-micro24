...
int main(int argc, char *argv[])
{
    ...
    for( int i = 0; i < 5; i ++){
        m5_setKernelSync(0, 0);
        printf ("info: Launch Init Kernel\n");
        m5_dump_reset_stats(0, 0);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(init_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, 0, A_d, B_d, C_d, initA, initB, initC);
        printf ("info: Launch Copy Kernel\n");
        m5_dump_reset_stats(0, 0);
      
        hipLaunchKernelGGL(HIP_KERNEL_NAME(copy_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, 0, A_d, C_d);
        printf ("info: Launch Multiply\n");
        m5_dump_reset_stats(0, 0);
        //m5_setKernelSync(0, 0);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(mul_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, 0, B_d, C_d);
        printf ("info: Launch Add Kernel\n");
        m5_dump_reset_stats(0, 0);
        //m5_setKernelSync(0, 0);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(add_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, 0, A_d, B_d, C_d);
        printf ("info: Launch Triad Kernel\n");
        m5_dump_reset_stats(0, 0);
        //m5_setKernelSync(1, 0);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(triad_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, 0, A_d, B_d, C_d);
        printf ("info: Launch Dot Kernel\n");
        m5_dump_reset_stats(0, 0);
        m5_setKernelSync(1, 1);
        hipLaunchKernelGGL(HIP_KERNEL_NAME(dot_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, 0, A_d, B_d, d_sum, array_size);
    }
    m5_dump_reset_stats(0, 0);
