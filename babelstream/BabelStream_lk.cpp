...
int main(int argc, char *argv[])
{
    ...
    CHECK ( hipMemcpy(C_d, C_h, Nbytes, hipMemcpyHostToDevice));
    int count = 257;
    for( int i = 0; i < 1; i ++){
        ...
        //fflush(stdout);
        m5_getKernelArg(reinterpret_cast<uintptr_t>(C_d), reinterpret_cast<uintptr_t>(A_d), 0, 3, 2, count++);
        hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(copy_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, hip_stream, 0, A_d, C_d);
        hipHccModuleRingDoorbell(hip_stream);
        hipStreamSynchronize(hip_stream);
	m5_dump_reset_stats(0, 0);
        printf ("info: Launch Multiply\n");
        //fflush(stdout);
        m5_getKernelArg(reinterpret_cast<uintptr_t>(C_d), reinterpret_cast<uintptr_t>(B_d), 0, 3, 2, count++);
        hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(mul_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, hip_stream, 0, B_d, C_d);
        hipHccModuleRingDoorbell(hip_stream);
        hipStreamSynchronize(hip_stream);
        m5_dump_reset_stats(0, 0);
        printf ("info: Launch Add Kernel\n");
        //fflush(stdout);
        m5_getKernelArg(reinterpret_cast<uintptr_t>(C_d), reinterpret_cast<uintptr_t>(A_d), reinterpret_cast<uintptr_t>(B_d), 3, 3, count++);
        hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(add_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, hip_stream, 0, A_d, B_d, C_d);
        hipHccModuleRingDoorbell(hip_stream);
        hipStreamSynchronize(hip_stream);
        m5_dump_reset_stats(0, 0);
        ...
        m5_getKernelArg(reinterpret_cast<uintptr_t>(A_d), reinterpret_cast<uintptr_t>(B_d), reinterpret_cast<uintptr_t>(C_d), 3, 3, count++);
        hipLaunchKernelGGL_lk(HIP_KERNEL_NAME(triad_kernel), dim3(array_size/TBSIZE), dim3(TBSIZE), 0, hip_stream, 0, A_d, B_d, C_d);
