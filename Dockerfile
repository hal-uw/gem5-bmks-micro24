...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    tcl-dev \
    tk-dev \
    python3-dev  doxygen libboost-all-dev \
...
RUN git clone https://github.com/hal-uw/ROCT-Thunk-Interface.git && \
...
RUN git clone https://@github.com/hal-uw/ROCR-Runtime.git && \
...
RUN git clone -b update https://@github.com/hal-uw/HIP.git && \
    mkdir -p /HIP/build
...
RUN git clone -b scheduling https://@github.com/hal-uw/MIOpen
...
RUN git -C /rocBLAS/ reset --hard cbff4b4e && git -C /rocBLAS/ apply /patch/rocBLAS.patch
RUN git -C /rocBLAS/ clean -f
RUN git -C /rocm-cmake/ checkout 12670acb
